...
#define nand_getpins ls1b_nand_getpins
#define nand_setio ls1b_nand_setio
#define nand_getio ls1b_nand_getio
#define nand_getbuswidth ls1b_nand_getbuswidth
...
#define PAGE_ADDR_SPARE(s) (((uint64_t)s->regs.addr_h<<(32 - 12))+((uint64_t)s->regs.addr_l>>12))
#define PAGE_ADDR_NOSPARE(s) (((uint64_t)s->regs.addr_h<<(32 - (12-1)))+((uint64_t)s->regs.addr_l>>(12-1)))
#define COLUM_ADDR_SPARE(s) (s->regs.addr_l&((1<<12)-1))
#define COLUM_ADDR_NOSPARE(s) (s->regs.addr_l&((1<<(12-1))-1))
...
static int nand_load_next(NandState *s)
{
	...
	{
		addr = PAGE_ADDR_SPARE(s) << s->chip->addr_shift;
		offset =  COLUM_ADDR_SPARE(s);
		iolen = s->chip->oob_size - offset;
	}
	else if((cmd & (CMD_SPARE |CMD_MAIN)) == (CMD_SPARE|CMD_MAIN))
	{
		addr = PAGE_ADDR_SPARE(s) << s->chip->addr_shift;
		offset =  COLUM_ADDR_NOSPARE(s);
		iolen = (1 << s->chip->page_shift) + s->chip->oob_size - offset;
	}
	else
	{
		addr = PAGE_ADDR_NOSPARE(s) << s->chip->addr_shift;
		offset =  COLUM_ADDR_NOSPARE(s);
...
static void ls1b_nand_do_cmd(NandState *s,uint32_t cmd)
{
	...
	if(cmd & CMD_ERASE)
	{
		if(cmd & CMD_BLKERASE)
		{
			...
			for(i=0,addr = PAGE_ADDR_NOSPARE(s)<<s->chip->addr_shift; i < s->regs.op_num; i++,addr += 1<<(s->chip->addr_shift+s->chip->erase_shift))
			{
				s->chip->addr = addr;
		...
		{
			s->chip->addr = PAGE_ADDR_NOSPARE(s)<<s->chip->addr_shift ;
			s->chip->blk_erase(s->chip);
		}
...
#define TYPE_SYS_BUS_LS1ANAND "ls1b_nand"
